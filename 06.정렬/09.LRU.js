...
// 만약캐시의사이즈가5이고 작업이 2 3 1 6 7 순으로저장되어있다면, (맨 앞이 가장 최근에 쓰인 작업이고, 맨 뒤는 가장 오랫동안 쓰이지 않은 작업이다.)

// 1) Cache Miss : 해야할 작업이 캐시에 없는 상태로 위 상태에서 만약 새로운 작업인 5번 작업을 CPU가 사용한다면 Cache miss가 되고
// 모든 작업이 뒤로 밀리고 5번작업은 캐시의 맨 앞에위치한다. 5 2 3 1 6 (7번작업은캐시에서삭제된다.)

// 2) Cache Hit : 해야할 작업이 캐시에 있는 상태로 위 상태에서 만약 3번 작업을 CPU가 사용한다면 Cache Hit가 되고,
// 6 3번 앞에 있는 5, 2번 작업은 한 칸 뒤로 밀리고, 3번이 맨 앞으로 위치하게 된다. 5 2 3 1 6 ---> 3 5 2 1 6
...
function solution(size, arr) {
  ...
  arr.forEach((x) => {
    ...
    for (let i = 0; i < size; i++) if (x === answer[i]) pos = i;
    if (pos === -1) {
      answer.unshift(x); // 맨 앞에 push
      if (answer.length > size) answer.pop(); // 사이즈보다 커지면 뒤에거 날리기
    } else {
      answer.splice(pos, 1);
      answer.unshift(x);
    }
